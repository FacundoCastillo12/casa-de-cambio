...
function mostrarTipoCambios(moneda){
    fetch(`https://v6.exchangerate-api.com/v6/502ae78ce62f17de5f000114/latest/${moneda}`)
    .then(respuesta => respuesta.json())
    .then(respuestaJSON => {
        $('h2').text(
            `Ultima actualizacion: ${ordenarFecha(respuestaJSON.time_last_update_utc)} en base ${respuestaJSON.base_code}`
        );
        $('li').remove();
        Object.keys(respuestaJSON.conversion_rates).forEach(moneda => {
            $('#lista-moneda').append($(`<li ${'class="list-group-item"'}>${moneda}: ${respuestaJSON.conversion_rates[moneda]}</li>`));
        })
    })
...
function convertirMoneda(monedaBase, monedaACambiar, cantidad){
    fetch(`https://v6.exchangerate-api.com/v6/502ae78ce62f17de5f000114/pair/${monedaBase}/${monedaACambiar}/${cantidad}`)
    .then(respuesta => respuesta.json())
    .then(respuestaJSON => {    
        $('h2').text(
            `Ultima actualizacion: ${ordenarFecha(respuestaJSON.time_last_update_utc)} convertir de ${respuestaJSON.base_code} a ${respuestaJSON.target_code}`
        )
        $('li').remove();
        $('#lista-moneda').append($(`<li class="list-group-item">Tasa de cambio de ${respuestaJSON.conversion_rate}</li>`));
        $('#lista-moneda').append($(`<li class="list-group-item">${respuestaJSON.base_code} a ${respuestaJSON.target_code}: ${respuestaJSON.conversion_result}</li>`));
    })
}
